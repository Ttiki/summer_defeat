...
void FPSPlayer::Start()
{
	...
	if (flashlighton)
	{
		auto temp = sound_flashlight;
		sound_flashlight = nullptr;
		ShowFlashlight();
...
bool FPSPlayer::Load(table& properties, shared_ptr<Stream> binstream, shared_ptr<Map> scene, const LoadFlags flags, shared_ptr<Object> extra)
{
	...
	if (properties["flashlighton"].is_boolean()) flashlighton = properties["flashlighton"];
	weapons.clear();
	weapons.resize(4);
	...
	if (properties["initialslot"].is_number())
	{
		initialslot = properties["initialslot"];
