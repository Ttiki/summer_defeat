	...
	float shadowSample0(in sampler2DArrayShadow shadowmap, in vec4 shadowcoord)
	{
		...
		float f = 2.0 * texture(shadowmap, shadowcoord + vec4(-sz, 0, 0.0, 0.0));
		f += 2.0 * texture(shadowmap, shadowcoord + vec4( sz, 0, 0.0, 0.0));
		f += texture(shadowmap, shadowcoord) * 1.0;
		return (f / 5.0);
	}
	...
	{
		float sz = 0.88888 / float(textureSize(shadowmap, 0).x);
		float f = 2.0 * texture(shadowmap, shadowcoord + vec4(0.0, -sz, 0.0, 0.0));
		f += 2.0 * texture(shadowmap, shadowcoord + vec4(0.0,  sz, 0.0, 0.0));
		f += texture(shadowmap, shadowcoord) * 1.0;
		return (f / 5.0);
	}
	...
	{
		float sz = 0.88888 / float(textureSize(shadowmap, 0).x);
		float f = 1.0 * texture(shadowmap, shadowcoord + vec4(-sz, -sz, 0.0, 0.0));
		f += 1.0 * texture(shadowmap, shadowcoord + vec4( sz,  sz, 0.0, 0.0));
		f += texture(shadowmap, shadowcoord) * 1.0;
		return (f) / 3.0;
	}
