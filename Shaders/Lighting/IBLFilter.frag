...
uniform float lodBias = 0.0f;
uniform uint sampleCount = 32;
uniform uint currentMipLevel = 0;
uniform uint width = 256;
uniform uint distribution = 0;
...
{
    //if (distribution == cGGX) color.a = 1.0f;
	if(face == 0)
		outFace0 = color;
	else if(face == 1)
		outFace1 = color;
...
void filterCube(in samplerCube tex)
{
...
void main() 
{
	...
	{
		outFace0 = vec4(1,0,1,1);
		outFace1 = vec4(1,0,1,1);
		outFace2 = vec4(1,0,1,1);
		outFace3 = vec4(1,0,1,1);
		outFace4 = vec4(1,0,1,1);
		outFace5 = vec4(1,0,1,1);
		return;
	}
	*/
    inUV = gl_FragCoord.xy / vec2(DrawViewport.z, DrawViewport.w);
    filterCube(envmap);
}